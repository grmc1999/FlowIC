...
class SpectralConv2d(nn.Module):
    ...
    def complex_mult(self, x, weights_real, weights_imag):
        """Complex multiplication in Fourier space"""
        return (x * weights_real - x * 1j * weights_imag)
...
class FNOBlock(nn.Module):
    ...
    def __call__(self, x):
        ...
        x_spectral = SpectralConv2d(self.width, self.width, self.modes1, self.modes2)(x)
        
        # Pointwise convolution in spatial domain
        x_spatial = nn.Conv(self.width, kernel_size=(1, 1))(x)
        ...
        x = nn.Conv(self.width, kernel_size=(1, 1))(x)
...
class FourierNeuralOperator(nn.Module):
    ...
    def __call__(self, x):
        """
        x: (batch_size, height, width, input_channels)
        Returns: (batch_size, height, width, output_channels)
        """
        # Initial lifting with a dense layer
        x = nn.Conv(self.width, kernel_size=(1, 1))(x)
        
        # Stack of FNO blocks
        for _ in range(self.depth):
            x = FNOBlock(self.width, self.modes1, self.modes2)(x)
...
if __name__ == "__main__":
    ...
    model = FourierNeuralOperator(modes1=12, modes2=12, width=32, depth=4)
